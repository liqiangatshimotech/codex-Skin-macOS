...
- JS 只主动标记侧栏、主区域、导航项、输入框和发送按钮。
- 首页只用于切换主区域主题状态和顶部装饰，不重排标题、建议卡或项目选择器。
- `findHeading`、`findHero` 和 `markCards` 的识别代码仍保留用于排查，但当前 `ensure()` 明确不启用标题、Hero 和卡片标记。
...
- 主区域：首页使用宽幅主题图；任务页使用更低对比的主题图叠层。
- 顶部装饰：铃铛品牌标识和 `TIME MACHINE READY` 状态。
- 输入区：保留官方尺寸，只增加边框、轻阴影、顶部三色细线和发送按钮配色。
...
- 重新启用标题、Hero 容器或建议卡 DOM 标记前，必须先解释为什么不会再次导致挤压或空白页。
